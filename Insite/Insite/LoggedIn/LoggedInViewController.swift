    ...
    func resetGeofences() {
        for region in locationManager.monitoredRegions {
            if region.identifier != "myLocation" {
                ...
                print("\(region.identifier) geofence deleted")
            }
        }
    }
        ...
        if mapView.annotations != nil {
            for annotation in mapView.annotations! {
                if annotation is MapAnnotation {
                    if (annotation as! MapAnnotation).type == "place" {
                        if (annotation as! MapAnnotation).place.name != id.dropLast() {
...
extension LoggedInViewController: MGLMapViewDelegate {
    func mapView(_ mapView: MGLMapView, viewFor annotation: MGLAnnotation) -> MGLAnnotationView? {
        
        if annotation.isKind(of: MapAnnotation.self) && (annotation as! MapAnnotation).type == "place" {
            let view = PlaceAnnotationView(reuseIdentifier: "placeAnnotation", place: (annotation as! MapAnnotation).place)
            view.rx
                .tapGesture()
                .when(.recognized)
                .subscribe(onNext: { [weak self] _ in
                    ...
                    self?.listener?.tappedOnPlaceAnnotation(place: (annotation as! MapAnnotation).place)
                })
                .disposed(by: disposeBag)
            return view
        }
...
extension LoggedInViewController: CLLocationManagerDelegate {
    func locationManager(_ manager: CLLocationManager, didUpdateLocations locations: [CLLocation]) {
        ...
        locationAdded = filterAndAddLocation(mostRecentLocation)
        ...
        if locationAdded {
            ...
            Firestore.firestore().collection("Users").document(Auth.auth().currentUser!.uid).collection("data").document("location").updateData(["history": FieldValue.arrayUnion([["location": GeoPoint(latitude: mostRecentLocation.coordinate.latitude, longitude: mostRecentLocation.coordinate.longitude), "time": Timestamp(date: Date()), "accuracy": mostRecentLocation.horizontalAccuracy]])])
            
            let geofenceRegion = CLCircularRegion(center: mostRecentLocation.coordinate, radius: 5, identifier: "myLocation")
    ...
    func filterAndAddLocation(_ location: CLLocation) -> Bool{
        ...
        if let curLoc = self.location {
            if location.distance(from: curLoc) < 5 {
                return false
            }
        }
