...
class HomeCollectionViewCell: UICollectionViewCell {
    ...
    func configure(with collectionViewModel: HomeCollectionData) -> (Void) {
        place = collectionViewModel.place
        
        imageView.image = place.image
        imageView.contentMode = .scaleAspectFill
        
        titleLabel.text = place.name.lowercased()
        
        if place.distance < 528 {
            subtitleLabel.text = "\(Int(place.distance)) ft"
        } else {
            subtitleLabel.text = "\(Double(round(10*(place.distance/5280))/10)) mi"
        }
