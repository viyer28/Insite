...
final class NavigationViewController: UIViewController, NavigationPresentable, NavigationViewControllable {
    ...
    var place: Place? {
        didSet {
            if let p = self.place {
                _setupNavigationView(place: p)
                for view in view.subviews {
                   view.alpha = 0
                }
                if let walkRoute = p.walkRoute {
                    if let driveRoute = p.driveRoute {
                        navigationEntranceAnimation(distance: p.distance, walkTime: walkRoute.expectedTravelTime, driveTime: driveRoute.expectedTravelTime)
                    } else {
                        navigationEntranceAnimation(distance: p.distance, walkTime: walkRoute.expectedTravelTime, driveTime: 0)
                    }
                } else {
                    if let driveRoute = p.driveRoute {
                        navigationEntranceAnimation(distance: p.distance, walkTime: 0, driveTime: driveRoute.expectedTravelTime)
                    } else {
                        navigationEntranceAnimation(distance: p.distance, walkTime: 0, driveTime: 0)
                    }
                }
            }
        }
    ...
    private func _setupNavigationView(place: Place) {
        ...
        distanceLabel.setUpdateBlock { [weak self] value, label in
            if value < 528 {
                ...
                label.text = "\(Double(round(10*(place.distance/5280))/10)) mi away"
            }
        ...
        walkingBackgroundView.rx
            ...
            .subscribe(onNext: { [weak self] _ in
                if self!.mode != "walking" {
                    ...
                    if let route = place.walkRoute {
                        self?.listener?.drawRoute(id: place.name + "w", route: route, dest: place.coordinate)
                    }
                    self?.mode = "walking"
                }
            })
        ...
        drivingBackgroundView.rx
            ...
            .subscribe(onNext: { [weak self] _ in
                if self!.mode != "driving" {
                    ...
                    if let route = place.driveRoute {
                        self?.listener?.drawRoute(id: place.name + "d", route: route, dest: place.coordinate)
                    }
                    self?.mode = "driving"
                }
            })
        ...
        directionsBackgroundView.rx
            ...
            .subscribe(onNext: { [weak self] _ in
                ...
                let alertController = UIAlertController(title: "", message: "\(place.address)", preferredStyle: .actionSheet)
                let uberAction = UIAlertAction(title: "Uber", style: .default) { (_) -> Void in
                    ...
                    builder.dropoffNickname = place.name
                ...
                let lyftAction = UIAlertAction(title: "Lyft", style: .default) { (_) -> Void in
                    LyftDeepLink.requestRide(kind: .Standard, to: place.coordinate)
                    alertController.dismiss(animated: true)
                }
                alertController.addAction(lyftAction)
                let appleAction = UIAlertAction(title: "Maps", style: .default) { [weak self] (_) -> Void in
                    let formattedAddress = place.address.replacingOccurrences(of: " ", with: "+")
                    if let url = URL(string:"http://maps.apple.com/?daddr=\(formattedAddress)&dirflg=\(String(self!.mode.first!))") {
                        UIApplication.shared.open(url)
                    } else if let llurl = URL(string:"http://maps.apple.com/?daddr=\(place.coordinate.latitude),\(place.coordinate.longitude)&dirflg=\(String(self!.mode.first!))") {
                ...
                let googleAction = UIAlertAction(title: "Google Maps", style: .default) { [weak self] (_) -> Void in
                    if (UIApplication.shared.canOpenURL(URL(string:"comgooglemaps://")!)) {
                        ...
                        if let url = URL(string:
                            ...
                            "comgooglemaps://?daddr=\(place.coordinate.latitude),\(place.coordinate.longitude)&directionsmode=\(self!.mode)") {
                            UIApplication.shared.open(llurl)
                        }
                    }
                    alertController.dismiss(animated: true)
                ...
                let wazeAction = UIAlertAction(title: "Waze", style: .default) { (_) -> Void in
                    if (UIApplication.shared.canOpenURL(URL(string:"waze://")!)) {
                        ...
                        if let url = URL(string: "waze://?q=\(formattedAddress)&navigate=yes") {
                            ...
                            "waze://?ll=\(place.coordinate.latitude),\(place.coordinate.longitude)&navigate=yes") {
                            UIApplication.shared.open(llurl)
                        }
                    }
                    alertController.dismiss(animated: true)
        ...
        shareImageView.rx
            ...
            .subscribe(onNext: { [weak self] _ in
                ...
                let copyGPSAction = UIAlertAction(title: "Copy GPS Coordinates", style: .default) { (_) -> Void in
                    UIPasteboard.general.string = "lat: \(place.coordinate.latitude), lon: \(place.coordinate.longitude)"
                    alertController.dismiss(animated: true)
                }
                ...
                let appleAction = UIAlertAction(title: "Open in Apple Maps", style: .default) { (_) -> Void in
                    ...
                    if let url = URL(string:"http://maps.apple.com/?address=\(formattedAddress)") {
                        UIApplication.shared.open(url)
                    } else  if let llurl = URL(string:"http://maps.apple.com/?address=\(place.coordinate.latitude),\(place.coordinate.longitude)") {
                        UIApplication.shared.open(llurl)
                    }
                    alertController.dismiss(animated: true)
                }
    ...
    private func navigationEntranceAnimation(distance: Double, walkTime: Double, driveTime: Double) {
        ...
        if walkTime == 0 {
            ...
            listener?.drawRoute(id: place!.name + "w", route: place!.walkRoute!, dest: place!.coordinate)
        }
        if driveTime == 0 {
